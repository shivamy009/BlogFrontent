...
const NotificationCard = ({data,index,notificationState,}) => {
    ...
    let {seen,createdAt,reply,comment,type,replied_on_comment,user,user:{personal_info:{profile_img,fullname,username}},blog:{_id,blog_id,title},_id:notification_id}=data;
    let {userAuth:{username:author_username,profile_img:author_profile_img,access_token}}=useContext(UserContext)
    let {notifications,notifications:{results},totalDocs,setNotification}=notificationState
    // console.log(data)
    ...
    const handleDelete=(comment_id,type,target)=>{
        ...
        .then(()=>{
            ...
            setNotification({...notificationState,results,totalDocs:totalDocs-1,deletedDocCount:notifications.deletedDocCount+1})
        })
        .catch(err=>{
            console.log(err)
        })
                ...
                <span className=' font-normal'>
                    {
                        ...
                        type=='comment' ? "commented your blog":
                        "replied on"
